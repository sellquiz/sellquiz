...
export class ParseCode {
    ...
    parseMatrixDef() {
        ...
        if(m <= 0)
            this.p.err("number of matrix cols must be > 0 (actually is '" + m + "')")
        // times
        this.p.terminal("x");
        // number of columns
        this.parseExpr();
        ...
        if(n <= 0)
            this.p.err("number of matrix rows must be > 0 (actually is '" + n + "')")
        // set
        this.p.terminal("|");
        this.parseExpr();
        ...
        this.p.terminal(")");
        // create symbol
        this.p.pushSym(symtype.T_MATRIX_DEF, [m, n, set, invertible, symmetric]);
    }
