...
export class ParseText {
    ...
    parseTitle() {
        ...
        this.p.q.html += '<div class="card-header">\n';
        this.p.q.html += '    <h5 class="card-title m-0">' + title + '</h5>\n';
        this.p.q.html += '    <a name="question-' + (this.p.questions.length-1) + '"></a>\n';
        this.p.q.html += '</div>\n';
        this.p.q.html += '<div class="card-body p-4">\n'; // ** begin body
    }
    ...
    parseText(parsingHint=false) {
        ...
        while(!this.p.is('§END') && !this.p.is('§CODE_START')) {
            ...
            if(this.p.is('§EOL') && this.p.isItemizeItem) {
                ...
                this.p.q.html += this.p.imParser.parseIM_Input();
    ...
    parseSingleMultipleChoice(isSingleChoice) { // TODO: permutation!
        ...
        let symId = (isSingleChoice?"_sc_":"_mc_") + this.p.createUniqueID();
        ...
        this.p.q.html += '\n§[';
        this.p.q.html += '<input id="' + inputId + '" type="' + inputType + '" name="sell_input" ' + checked + '/>&nbsp;';

        this.p.singleMultipleChoiceFeedbackHTML = '&nbsp;<span id="sell_input_feedback_' + this.p.instanceID + '_' + this.p.qidx + '_' + symId + '"></span>\n';
    }
