...
export class SellLinAlg {
    ...
    static mat_norm2(mathjs_matrix) {
        ...
        for (let i = 0; i < m; i++) {
            for (let j = 0; j < n; j++) {
                let e_ij = (mathjs_matrix as math.Matrix).subset(math.index(i, j)) as any;
                res += e_ij * e_ij;
            }
        }
    ...
    static mat_kernel(mathjs_matrix) {
        ...
        let lup = math.lup(mathjs_matrix) as any;
        let u = lup["U"];
        let m = u.size()[0];
        let n = u.size()[1];
        let v = [];
        for (let i = 0; i < m; i++)
            for (let j = 0; j < n; j++)
                v.push((u as math.Matrix).subset(math.index(i, j)));
        ...
        for (let i = 0; i < dest_m; i++) {
            for (let j = 0; j < dest_n; j++) {
                dest_mathjs = (dest_mathjs as math.Matrix).subset(math.index(i, j), dest[this.mat_idx(dest_m, dest_n, i, j)]);
            }
        }
